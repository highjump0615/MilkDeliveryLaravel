	...
	<div class="border-bottom white-bg">
		<div class="col-lg-6 col-md-6 " style="padding-top:5px;">
			...
			<a type="button" href="{{url('/naizhan/xiaoxi/zhongxin')}}" class="btn btn-outline btn-success btn-xs col-xs-3">
				消息中心 &nbsp;
				<span id="notification" class="label label-danger">@if (count(\App\Model\NotificationModel\DSNotification::where('station_id',Auth::guard('naizhan')->user()->station_id)->where('read',0)->get()) > 0){{count(\App\Model\NotificationModel\DSNotification::where('station_id',Auth::guard('naizhan')->user()->station_id)->where('read',0)->get())}}@endif</span>
			</a>
			</p>
		</div>
