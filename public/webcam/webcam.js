...
var Webcam = {
	...
	attach: function(elem) {
		...
		if (this.userMedia) {
			...
			function(err) {
				return self.dispatch('error', "无法访问网络摄像头.");
			});
		}
		else {
			// flash fallback
	...
	snap: function(user_callback, user_canvas) {
		...
		if (!this.loaded) return this.dispatch('error', "网络摄像头尚未加载");
		if (!this.live) return this.dispatch('error', "网络摄像头还没有上线");
		if (!user_callback) return this.dispatch('error', "请提供一个回调函数或画布到snap（）");
