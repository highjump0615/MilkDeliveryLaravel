    ...
    MobileArea.prototype = {
        ...
        bindEvent: function() {
            ...
            function popupArea(e) {
                ...
                var strInnerHtml = '<div class="area_ctrl slideInUp">' +
                ...
                _self.gearArea.innerHTML = strInnerHtml;
             ...
             * @returns {number}
             */
            function getValueIndexInArray(valuesSrc, value, isValue) {
                var i = 0;
                for (i = 0; i < valuesSrc.length; i++) {
                    // 搜索对象为value
                    if (isValue) {
                        if (valuesSrc[i].value === value) {
                            break;
                        }
                    }
                    // 搜索对象为text
                    else {
                        if (valuesSrc[i].text === value) {
                            break;
                        }
                    }
                }

                return i >= valuesSrc.length ? -1 : i;
            }
             ...
             * @returns {number}
             */
            function getValueIndex(valuesSrc, value, isValue) {

                var nRes = 0;
                if (Array.isArray(valuesSrc)) {
                    nRes = getValueIndexInArray(valuesSrc, value, isValue);
                }
                else {
                    for (var key in valuesSrc) {
                        nRes = getValueIndexInArray(valuesSrc[key], value, isValue);
                        if (nRes >= 0) {
                            break;
                        }
                    }
                ...
                return Math.max(nRes, 0);
            }

            //初始化插件默认值
            function areaCtrlInit() {
                _self.gearArea.querySelector(".area_province").setAttribute("val", getValueIndex(_self.data[0], _self.value[0], true));
                _self.gearArea.querySelector(".area_city").setAttribute("val", getValueIndex(_self.data[1], _self.value[1], true));

                var area_county = _self.gearArea.querySelector(".area_county");
                if (area_county) {
                    area_county.setAttribute("val", getValueIndex(_self.data[2], _self.value[2], true));
                }
        ...
        setGearTooth: function(data) {
            ...
            if (l > 0) {
                ...
                _self.index++;
                if (_self.index >= _self.data.length) {
                    _self.index = 0;
                    return;
                }
                _self.setGearTooth(childData);
        ...
        finish: function(e) {
            ...
            _self.value = [provinceCode, cityCode, countyCode];
            if(this.valueTo){
                this.valueTo.value= provinceCode +((cityCode)?(',' + cityCode):('')) + ((countyCode)?(',' + countyCode):(''));
            }

            // call to success event handler
            if (_self.onSuccess) {
                _self.onSuccess();
            }
            _self.close(e);
