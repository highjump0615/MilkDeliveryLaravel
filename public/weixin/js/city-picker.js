...
(function (factory) {
    ...
    CityPicker.prototype = {
        ...
        render: function () {
            ...
            var placeholder = this.$element.attr('placeholder') || this.options.placeholder,
                ...
                dropdown = '<div class="city-picker-dropdown" style="left:0px;top:100%;' +
                    ...
                    (this.includeDem('street') ? '<a data-count="street">街道</a>' : '') +
                    (this.includeDem('xiaoqu') ? '<a data-count="xiaoqu">小区</a>' : '') + '</div>' +
                    ...
                    (this.includeDem('xiaoqu') ? '<div class="city-select xiaoqu" data-count="xiaoqu"></div>' : '') +
                    '</div></div>';
        ...
        defineDems: function () {
            var stop = false;
            $.each([PROVINCE, CITY, DISTRICT, STREET, XIAOQU], $.proxy(function (i, type) {
                if (!stop) {
                    this.dems.push(type);
                }
        ...
        bind: function () {
            ...
            if (this.$street) {
                this.$street.on(EVENT_CHANGE, (this._changeStreet = $.proxy(function () {
                    this.output(XIAOQU);
                    this.tab(XIAOQU);
                }, this)));
            }
                ...
                type === PROVINCE ? 86 :
                    type === CITY ? this.$province && this.$province.find('.active').data('code') :
                        type === DISTRICT ? this.$city && this.$city.find('.active').data('code') :
                            type === STREET ? this.$district && this.$district.find('.active').data('code') :
                                type === XIAOQU ? this.$street && this.$street.find('.active').data('code') : code
            );
